...
- [Configuration at the target snapshot](#configuration-at-the-target-snapshot)
...
## Configuration at the target snapshot

Read the target commit's configuration module document, configuration code,
pipeline registry, deploy schemas, and tests together. Respect that document's
status: a deferred or draft ownership proposal cannot supply merge-blocking
invariants on its own.

The [v0.28.0 maintenance record](../../update-vllm-omni-skills/references/release-status.md)
records a draft configuration contract and the pipeline/deploy migration at that
release. For another target, resolve the corresponding evidence again. Do not
require historical resolver symbols, serialized config shapes, or legacy YAML
adapters merely because an earlier skill described them.

Trace a configuration change from caller input through defaults/overrides,
validation, stage construction, transport, and engine initialization. Ground a
finding in a concrete behavior regression or an accepted target contract, and
request focused tests for the changed precedence, topology, or runtime boundary.
...
| Configuration | `vllm_omni/config/**`, `vllm_omni/deploy/**`, `vllm_omni/entrypoints/utils.py`, `vllm_omni/engine/stage_runtime.py` | Trace the target snapshot's input resolution, validation, transport, and engine initialization; derive required boundaries from its accepted contracts and tests | `tests/config/**`, affected launch tests |
