...
### Repository contribution decision

Canonical source: `skills/vllm-omni-review/` in this repository.

For a repository-ready improvement:

- Check this repository's contribution instructions and current skill content before editing.
- Make a narrow patch, validate the skill frontmatter and relative links, run `bash -n skills/vllm-omni-review/scripts/*.sh`, and explain the triggering feedback and why it generalizes.
- Decide: **no change**, **prepare draft**, or **submit PR**.
- Choose **no change** for contradictory, user-specific, or weakly evidenced guidance.
- Choose **prepare draft** when the change is durable but repository submission is not authorized.
- Submit a PR only when the user explicitly authorizes submission for the current session or has supplied a standing policy that covers this change. Never silently rewrite the skill or publish a PR.
